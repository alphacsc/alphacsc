...
class ConvolutionalDictionaryLearning(TransformerMixin):
    ...
    def fit(self, X, y=None):
        self.pobj_, self.times_, self.D_hat, self.z_hat = learn_d_z_multi(
            ...
            stopping_pobj=self.stopping_pobj,
            algorithm=self.algorithm, alpha=self.alpha,
            batch_size=self.batch_size, batch_selection=self.batch_selection,
...
class BatchCDL(ConvolutionalDictionaryLearning):
    ...
    def __init__(self, n_atoms, n_times_atom, reg=0.1, n_iter=60, n_jobs=1,
                 solver_z='lgcd', solver_z_kwargs=dict(),
                 solver_d='alternate_adaptive', solver_d_kwargs=dict(),
                 rank1=True, uv_constraint='separate',
                 eps=1e-10, D_init=None, D_init_params=dict(),
...
class OnlineCDL(ConvolutionalDictionaryLearning):
    ...
    def __init__(self, n_atoms, n_times_atom, reg=0.1, n_iter=60, n_jobs=1,
        ...
        super().__init__(
            ...
            batch_selection=batch_selection, n_jobs=n_jobs,
            random_state=random_state, algorithm='online', lmbd_max='fixed',
            raise_on_increase=True, loss='l2', verbose=10, callback=None,
            stopping_pobj=None, use_sparse_z=False, name="OnlineCDL")
