...
@mem.cache()
def load_data(n_jobs=1, sfreq=sfreq):
...
def get_callback(n_atoms):
    """Create a callback to display at each iteration the atoms and activation
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--callback', action='store_true',
                        help='Plot at each iteration the atoms and activation '
                        'signals learned so far.')
    ...
    callback_ = None
    if args.callback:
        callback_ = get_callback(n_atoms)
    ...
    if args.multi:
        ...
        pobj, times, uv_hat, Z_hat = learn_uv_z(
            ...
            uv_init='kmeans', uv_constraint='separate',
            callback=callback_, algorithm='batch')
